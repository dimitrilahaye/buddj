...
export default class TypeOrmMonthRepository implements MonthRepository {
    async addExpenseToWeeklyBudget(month: Month, weeklyId: string, expense: WeeklyExpense): Promise<void> {
        const weekly = month.account.weeklyBudgets.find((weekly) => weekly.id === weeklyId);
        const weeklyDao = await WeeklyBudgetDao.findOne({where: {id: weeklyId}});
        if (weekly && weeklyDao) {
    ...
    async addOutflow(month: Month, outflow: AccountOutflow): Promise<void> {
        const accountDao = await AccountDao.findOne({where: {id: month.account.id}});
    ...
    async manageOutflowsChecking(outflows: {
        id: string;
        isChecked: boolean;
    }[]): Promise<void> {
    ...
    async manageExpensesChecking(month: Month, weeklyBudgets: {
        id: string,
        expenses: { id: string, isChecked: boolean }[],
    }[]): Promise<void> {
        for (const {id, expenses} of weeklyBudgets) {
            const weekly = month.account.weeklyBudgets.find((weekly) => weekly.id === id);
            if (weekly) {
    ...
    async updateWeeklyBudgetCurrentBalance(month: Month, weeklyId: string): Promise<void> {
        const weekly = month.account.weeklyBudgets.find((weekly) => weekly.id === weeklyId);
        if (weekly) {
