...
describe('Unit | Core | Usecases | ManageOutflowsChecking', function () {
    it('should return an up to date Month', async function () {
        ...
        // then
        expect(month.updateAccountCurrentBalance.calledOnceWithExactly(command.currentBalance)).to.be.true;
        expect(month.checkOutflow.calledOnceWithExactly(outflowIdForCheck)).to.be.true;
        expect(month.checkOutflow.calledOnceWithExactly(outflowIdForCheck)).to.be.true;
        expect(monthRepositoryStub.updateAccountCurrentBalance.calledOnceWithExactly(month)).to.be.true;
        expect(monthRepositoryStub.manageOutflowsChecking.calledOnceWithExactly(command.outflows)).to.be.true;
