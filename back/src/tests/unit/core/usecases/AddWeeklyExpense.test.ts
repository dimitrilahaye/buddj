...
describe("Unit | Core | Usecases | AddWeeklyExpense", function () {
    it("should return a new added weekly expense", async function () {
        ...
        const usecase = new AddWeeklyExpense(weeklyExpenseFactory, monthRepositoryStub);
        ...
        expect(month.addExpenseToWeeklyBudget.calledOnceWithExactly(command.weeklyBudgetId, expense)).to.be.true;
        expect(monthRepositoryStub.addExpenseToWeeklyBudget.calledOnceWithExactly(month, command.weeklyBudgetId, expense)).to.be.true;
        expect(monthRepositoryStub.updateWeeklyBudgetCurrentBalance.calledOnceWithExactly(month, command.weeklyBudgetId)).to.be.true;
