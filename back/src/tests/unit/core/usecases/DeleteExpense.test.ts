...
describe("Unit | Core | Usecases | DeleteExpense", function () {
    ...
    it("should return the month after expense deletion", async function () {
        ...
        // then
        expect(foundMonth.deleteExpenseFromWeeklyBudget).to.have.been.calledOnceWith(command.weeklyId, command.expenseId);
        expect(monthRepositoryStub.deleteExpense).to.have.been.calledOnceWith(command.expenseId);
        expect(monthRepositoryStub.updateWeeklyBudgetCurrentBalance).to.have.been.calledOnceWith(foundMonth, command.weeklyId);
