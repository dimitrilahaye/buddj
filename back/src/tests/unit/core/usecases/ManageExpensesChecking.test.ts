...
describe('Unit | Core | Usecases | ManageExpensesChecking', function () {
    it('should return an up to date Month', async function () {
        ...
        // then
        expect(month.uncheckExpense.calledOnceWithExactly(weeklyIdForUncheck, expenseIdForUncheck)).to.be.true;
        expect(month.checkExpense.calledOnceWithExactly(weeklyIdForCheck, expenseIdForCheck)).to.be.true;
        expect(monthRepositoryStub.manageExpensesChecking.calledOnceWithExactly(month, command.weeklyBudgets)).to.be.true;
        for (const {id} of command.weeklyBudgets) {
            expect(monthRepositoryStub.updateWeeklyBudgetCurrentBalance.calledWith(month, id)).to.be.true;
        }
