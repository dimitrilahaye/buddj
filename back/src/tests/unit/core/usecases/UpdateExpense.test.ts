...
describe('Unit | Core | Usecases | UpdateExpense', function () {
    it('should update the expense and return the updated month', async function () {
        ...
        // then
        expect(foundMonth.updateExpenseAmountFromWeeklyBudget).to.have.been.calledOnceWith(command.originalWeeklyId, command.expenseId, command.amount);
        expect(foundMonth.updateExpenseLabelFromWeeklyBudget).to.have.been.calledOnceWith(command.originalWeeklyId, command.expenseId, command.label);
        expect(foundMonth.updateExpenseWeeklyBudget).to.have.been.calledOnceWith(command.originalWeeklyId, command.newWeeklyId, command.expenseId);
