...
describe("Integration | Consumers | Routes | DELETE /months/{monthId}", function () {
  ...
  describe("When user is authenticated", function () {
    ...
    it("should return a 200 on happy path", async function () {
      ...
      depsStub.deleteMonthUsecase.execute = sinon
        .stub()
        .withArgs(command)
        .resolves([archivedMonth]);
      ...
      // then
      expect(depsStub.deleteMonthUsecase.execute as SinonStub).to.have.been
        .calledOnce;
    ...
    it("should return 404 error if a model is not found", async function () {
      ...
      deps.deleteMonthUsecase.execute = sinon
        .stub()
        .withArgs(command)
      ...
      // then
      expect(deps.deleteMonthUsecase.execute as SinonStub).to.have.been
        .calledOnce;
  ...
  describe("When user is not authenticated", function () {
    ...
    it("should return a 401 error", async function () {
      await request(server)
        .delete("/months/85467999-2751-4f04-8d96-7d7727fbff02")
        .expect(401);
    });
  });
});
