...
describe("Integration | Consumers | Routes | PUT /months/{monthId}/weekly/{weeklyId}/expenses/{expenseId}", function () {
    ...
    describe("When user is authenticated", function () {
        ...
        it("should return a 200 on happy path", async function () {
            ...
            depsStub.updateExpenseUsecase.execute = sinon.stub().withArgs(command).resolves(updatedMonth);

            server = mockedServer({isAuthenticated: true}, depsStub);
            const cookie = await authenticate(server);
            const {monthId, originalWeeklyId: weeklyId, expenseId, ...body} = command;
        ...
        it("should return 422 error if a core error is thrown", async function () {
            ...
            deps.updateExpenseUsecase.execute = sinon.stub().withArgs(command).rejects(new WeeklyExpenseAmountError());
        ...
        it("should return 404 error if a model is not found", async function () {
            ...
            deps.updateExpenseUsecase.execute = sinon.stub().withArgs(command).throwsException(new MonthNotFoundError());
    ...
    describe("When user is not authenticated", function () {
        ...
        it("should return a 401 error", async function () {
            await request(server)
                .put('/months/85467999-2751-4f04-8d96-7d7727fbff02/weekly/85467999-2751-4f04-8d96-7d7727fbff02/expenses/85467999-2751-4f04-8d96-7d7727fbff02')
                .expect(401);
        });
    });
});
