...
describe("Integration | Consumers | Routes | PUT /months/{monthId}/archive", function () {
    ...
    describe("When user is authenticated", function () {
        ...
        it("should return a 200 on happy path", async function () {
            ...
            depsStub.archiveMonthUsecase.execute = sinon.stub().withArgs(command).resolves(updatedMonth);
        ...
        it("should return 404 error if a model is not found", async function () {
            ...
            deps.archiveMonthUsecase.execute = sinon.stub().withArgs(command).throwsException(new MonthNotFoundError());
    ...
    describe("When user is not authenticated", function () {
        ...
        it("should return a 401 error", async function () {
            await request(server).put('/months/85467999-2751-4f04-8d96-7d7727fbff02/archive').expect(401);
        });
    });
});
