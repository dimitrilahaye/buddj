...
describe("Integration | Consumers | Routes | PUT /months/{monthId}/unarchive", function () {
    ...
    describe("When user is authenticated", function () {
        ...
        it("should return a 200 on happy path", async function () {
            ...
            depsStub.unarchiveMonthUsecase.execute = sinon.stub().withArgs(command).resolves([unarchivedMonth]);
            ...
            // then
            expect((depsStub.unarchiveMonthUsecase.execute as SinonStub)).to.have.been.calledOnce;
        ...
        it("should return 404 error if a model is not found", async function () {
            ...
            deps.unarchiveMonthUsecase.execute = sinon.stub().withArgs(command).throwsException(new MonthNotFoundError());
            ...
            // then
            expect((deps.unarchiveMonthUsecase.execute as SinonStub)).to.have.been.calledOnce;
    ...
    describe("When user is not authenticated", function () {
        ...
        it("should return a 401 error", async function () {
            await request(server).put('/months/85467999-2751-4f04-8d96-7d7727fbff02/unarchive').expect(401);
        });
    });
});
