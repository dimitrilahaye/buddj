...
describe("Integration | Providers | Persistence | Repositories | MonthRepository", function () {
    ...
    describe("#findAllUnarchived", function () {
        it("should return all unarchived months", async function () {
            ...
            expect(foundUnarchivedMonths[0].id).to.be.equal(persistedUnarchivedMonth.id);
    ...
    describe('#addExpenseToWeeklyBudget', () => {
        it('should persist the new expense', async () => {
            ...
            // then
            const persistedExpense = await WeeklyExpenseDao.findOneByOrFail({id: newExpense.id});
            expect(persistedExpense).not.to.be.null;
            const persistedWeeklyBudget = await WeeklyBudgetDao.findOneByOrFail({id: weeklyId});
            const foundExpenseIntoWeeklyBudget = persistedWeeklyBudget.expenses.find((expense) => expense.id === persistedExpense.id);
    ...
    describe('#addOutflow', () => {
        it('should add outflow', async () => {
            ...
            // then
            const persistedOutflow = await OutflowDao.findOneByOrFail({id: newOutflow.id});
            expect(persistedOutflow).not.to.be.null;
            const persistedAccount = await AccountDao.findOneByOrFail({id: month.account.id});
            const foundOutflowIntoAccount = persistedAccount.outflows.find((outflow) => outflow.id === persistedOutflow.id);
    ...
    describe('#deleteExpense', () => {
        it('should delete the expense', async () => {
            ...
            // then
            const persistedExpense = await WeeklyExpenseDao.findOneBy({id: expense.id});
    ...
    describe('#manageOutflowsChecking', () => {
        it('should check and uncheck outflows', async () => {
            ...
            // then
            const persistedOutflowToCheck = await OutflowDao.findOneByOrFail({id: outflowToCheck.id});
            expect(persistedOutflowToCheck.isChecked).to.be.true;
            const persistedOutflowToUncheck = await OutflowDao.findOneByOrFail({id: outflowToUncheck.id});
    ...
    describe('#manageExpensesChecking', () => {
        it('should check and uncheck expenses', async () => {
            ...
            // then
            const persistedExpenseToCheck = await WeeklyExpenseDao.findOneByOrFail({id: expenseToCheck.id});
            expect(persistedExpenseToCheck.isChecked).to.be.true;
            const persistedExpenseToUncheck = await WeeklyExpenseDao.findOneByOrFail({id: expenseToUncheck.id});
    ...
    describe('#updateWeeklyBudgetCurrentBalance', () => {
        it('should update the current balance of the given weekly budget', async () => {
            ...
            // then
            const updatedWeeklyBudget = await WeeklyBudgetDao.findOneByOrFail({id: weekly.id});
    ...
    describe('#delete', () => {
        it('should delete the month', async () => {
            ...
            const persistedAccount = await AccountDao.findOneBy({id: month.account.id});
