...
describe('transfert budget -> budget/account', () => {
  beforeEach(() => {
  ...
  it('valide drawer, Max, Remettre à 0, loading, toast succès et refresh vue', async () => {
        ...
        budgetGroups: [
          {
            ...
            budgets: [
              ...
              },
              {
                ...
                expenses: [{ id: 'exp-b', icon: '💰', desc: 'Courses', amount: 150, taken: false }],
              },
            ],
          },
        ],
    ...
    await waitFor(() => {
      transferDrawer = screen.getByRole('dialog', { name: 'Transférer' }) as HTMLElement;
      expect(transferDrawer).toBeTruthy();
    });
    transferDrawer = screen.getByRole('dialog', { name: 'Transférer' }) as HTMLElement;
    ...
    const budgetDestination = Array.from(destinationButtons).find((btn) => (btn as HTMLElement).dataset.destinationId === weekB);
    const accountDestination = Array.from(destinationButtons).find((btn) => (btn as HTMLElement).dataset.destinationId === accountId);
    expect(budgetDestination).toBeTruthy();
    expect(accountDestination).toBeTruthy();
    expect(within(budgetDestination as HTMLElement).getAllByText(formatEuros(-50)).length).toBeGreaterThan(0);
    ...
    fireEvent.click(budgetDestination as Element);
    ...
    const sourceCardAfter = screen.getByRole('heading', { name: 'Semaine 123', level: 2 }).closest('buddj-budget-card') as HTMLElement;
    const destinationCardAfter = screen.getByRole('heading', { name: 'Semaine 3', level: 2 }).closest('buddj-budget-card') as HTMLElement;
    expect(sourceCardAfter.getAttribute('remaining')).toBe('137');
