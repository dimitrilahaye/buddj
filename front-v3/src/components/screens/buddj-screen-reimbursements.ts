...
export class BuddjScreenReimbursements extends HTMLElement {
  ...
  private _injectionSourceAmount = 0;
  ...
  init({ reimbursementsStore, monthStore }: { reimbursementsStore: ReimbursementsStore; monthStore: MonthStore }): void {
  ...
  private _onAddAmountLoaded = ((e: Event): void => {
    ...
    if (this._pendingInjectedAmount > 0 && detail?.projectId === this._pendingInjectedProjectId) {
      ...
      if (this._injectionRemainingAmount <= 0) {
        ...
        this.showToast(
          `Votre solde prévisionnel est à 0. Vous venez de transférer ${formatEuros(this._injectionTransferredTotal)} dans vos remboursements.`
        );
      }
    }
  ...
  private render(): void {
        ...
        <div class="goal-section-header">
          ...
          <buddj-actions-dropdown data-goal-injection-menu position="right">
            ...
            >
              ...
              <span class="goal-injection-dropdown-text">Vous ne savez pas quoi faire de votre solde prévisionnel ? Et si vous vous en serviez pour boucler vos remboursements ?</span>
            </button>
    ...
    reminder?.renderReminder?.({ totalTransferred: this._reminderTransferredTotal, targetLabel: 'vos remboursements' });
  ...
  private startInjectionFlow(): void {
    ...
    this._injectionSourceAmount = projectedBalance;
  ...
  private openInjectionDrawer(): void {
    ...
    drawer?.open({
      title: 'dans vos remboursements',
      selectedAmount: this._injectionTemporaryAmount,
      balanceAmount: this._injectionRemainingAmount,
      projects,
      emptyLabel: 'Aucun remboursement disponible pour une injection.',
      onClose: () => {
        this.closeInjectionDrawer({ showReminder: true });
      },
      onAmountClick: () => {
